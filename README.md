...
- Deployment-oriented live-loop smoke command that applies migrations, seeds one deterministic approved micro-size LIVE risk decision, validates the live-order-plan/live-readiness/Kill Switch/live-loop-audit/deploy-check/live-ops-report handoff chain, runs the real bounded app/db pipeline against a fake exchange adapter, and verifies durable risk/order/status/live-loop audit rows without contacting Bybit or placing a real order.
...
- Read-only live readiness CLI that checks live config/env/operator confirmation, capital cap, pending selector DB-pool safety, inactive Kill Switch, pending LIVE decision availability, optional live order plan artifact freshness, and recent live-loop audit status without contacting Bybit or placing orders, with an optional durable JSON checklist artifact.
- Operator-facing risk Kill Switch CLI that can inspect current persistent state, list audit events, and explicitly append activate/release events with required reasons.
...
- Operator-facing live first-order check CLI that builds a reproducible Kill Switch state, plan/readiness/audit/deploy-check/ops-report artifact bundle, runs the final offline handoff verification, and prints the exact armed `live-loop` command with Kill Switch and ops-report gates without running the order-capable step.
...
- Read-only-by-default live ops report CLI that summarizes Kill Switch state, pending LIVE decisions, recent live-loop audit verdict, optional first-order review artifact freshness, and optional exchange-vs-DB position drift into `CLEAR`, `ATTENTION`, or `BLOCKED`, with an explicit opt-in Kill Switch activation flag for blocked drift.
- Read-only-by-default live position drift CLI that compares fresh exchange position snapshots with the latest persisted DB baselines, ignores market-noise fields such as mark price/PnL, blocks on exposure drift or abnormal exchange position status, and can explicitly persist an active Kill Switch event on blocked drift.
...
Automatically reconcile open paper exits by sourcing a fresh orderbook quote and settling only positions whose mid price has crossed stop-loss or take-profit. If no position is open or no exit trigger is hit, the command logs a skipped result and exits successfully:
...
Run the Docker-backed smoke for the bounded cycle. It applies migrations, seeds a repeatable RUNNING paper validation with one approved paper ticket and fresh orderbook snapshots, then verifies active Kill Switch entry blocking, entry, duplicate-entry prevention while the position is active, take-profit close, equity-ledger accounting, and settled validation completion. The scripts write a temporary paper-enabled config and do not modify `configs/config.example.yaml`:
...
Run the read-only live readiness checklist before contacting Bybit or arming a live-loop order path. It checks config/env/operator confirmation, DB pool safety for pending selection, inactive Kill Switch, pending decision availability, optional plan artifact freshness, and recent live-loop audit state. It uses only PostgreSQL and local environment variables; pass `-artifact-path` when you want a durable JSON checklist for deployment/audit handoff:

```powershell
go run ./cmd/live-readiness -config configs/live.local.yaml -symbol BTCUSDT -subaccount-confirmed -max-initial-live-capital-usdt 100 -artifact-path artifacts/live-readiness.json
```
...
```powershell
go run ./cmd/live-readiness -config configs/live.local.yaml -symbol BTCUSDT -plan-file artifacts/live-order-plan.json -subaccount-confirmed -max-initial-live-capital-usdt 100 -artifact-path artifacts/live-readiness.json
```
...
Run the bounded live-loop health command before wiring any autonomous iteration source. It reruns the same startup preflight, records the same audit snapshots, checks the Kill Switch before a no-op loop iteration, and logs `healthy`, `stop_reason`, and iteration counters without creating signals or placing orders:
...
Run the deployment smoke before the first armed live-loop order path. It applies migrations, seeds one deterministic approved micro-size LIVE risk decision, builds and validates the same live-order-plan, live-readiness, inactive Kill Switch state, live-loop-audit, live-deploy-check, and CLEAR live-ops-report handoff chain used by the operator runbook, uses the real app use cases and PostgreSQL repositories, and verifies persisted risk-decision, live-order, status, and live-loop audit rows. The exchange adapter is fake in this smoke, so it never contacts Bybit and never places a real order:

```powershell
$env:DATABASE_DSN="postgres://inquisitor:inquisitor@localhost:5432/inquisitor?sslmode=disable"
go run ./cmd/live-loop-smoke -config configs/config.example.yaml -migrations migrations -subaccount-confirmed -execute
```
...
For the safest handoff from preview to execution, pass the JSON artifacts to the armed `live-loop`: the order plan from `live-order-plan`, the PASS checklist from `live-readiness`, the recent `live-loop-audit` artifact, optionally the inactive Kill Switch state snapshot from `risk-kill-switch`, optionally the PASS go/no-go report from `live-deploy-check`, and optionally a fresh CLEAR `live-ops-report` artifact. Use an explicit `-decision-id` artifact with explicit execution, or a FIFO `-select-pending` artifact with `live-loop -select-pending`; mixed source modes are rejected before configuration, database, or exchange setup. `live-handoff-verify`, `live-deploy-check`, and armed `live-loop` verify audit artifact freshness plus audit summary/verdict consistency against the readiness artifact before any DB or exchange-capable command runs. `live-handoff-verify -kill-switch-file` and armed `live-loop -kill-switch-file` verify that the Kill Switch artifact is fresh, inactive, config-matched, and consistent with the readiness artifact when readiness is supplied. When `-kill-switch-file` is passed, `live-loop` rejects stale Kill Switch artifacts (`-max-kill-switch-age`, `10m` by default), active state, config mismatch, and readiness mismatch before config, database, or exchange-capable setup. `live-deploy-check` is the stricter final dry gate for the first live micro order: it also requires `-execute`, `-subaccount-confirmed`, one iteration, no readiness warnings, CLEAR audit review, max initial live capital and planned notional within the live micro limit, and planned leverage no higher than `1`; pass `-artifact-path` to persist a machine-readable go/no-go report even when the gate fails. When `-deploy-check-file` is passed, `live-loop` also rejects stale deployment-check artifacts (`-max-deploy-check-age`, `10m` by default), failed/warned deployment reports, and any mismatch between the saved go/no-go report and the actual plan/readiness/audit files or execution flags. `live-handoff-verify -ops-report-file` and armed `live-loop -ops-report-file` reject stale ops reports (`-max-ops-report-age`, `10m` by default), config mismatches, and any `ATTENTION` or `BLOCKED` ops status; `live-loop` does this before config, database, or exchange-capable setup. `live-readiness` records the exact plan-file SHA-256 in the checklist, and `live-loop` recomputes it before execution. `live-loop` rejects stale readiness artifacts (`-max-readiness-age`, `10m` by default), stale plan artifacts (`-max-plan-age`, `10m` by default), stale audit artifacts (`-max-audit-age`, `10m` by default), readiness FAIL/checklist mismatches, audit verdict mismatches, plan metadata or SHA-256 mismatches, and any current PostgreSQL risk snapshot drift before startup preflight or exchange setup:

```powershell
go run ./cmd/live-handoff-verify -config configs/live.local.yaml -plan-file artifacts/live-order-plan.json -readiness-file artifacts/live-readiness.json -kill-switch-file artifacts/risk-kill-switch-state.json -audit-file artifacts/live-loop-audit.json -deploy-check-file artifacts/live-deploy-check.json -ops-report-file artifacts/live-ops-report.json -decision-id risk_decision_live_001 -subaccount-confirmed -max-initial-live-capital-usdt 100 -max-iterations 1 -max-runtime 15s -iteration-timeout 10s -execute
```

```powershell
go run ./cmd/live-deploy-check -config configs/live.local.yaml -plan-file artifacts/live-order-plan.json -readiness-file artifacts/live-readiness.json -audit-file artifacts/live-loop-audit.json -artifact-path artifacts/live-deploy-check.json -decision-id risk_decision_live_001 -subaccount-confirmed -max-initial-live-capital-usdt 100 -max-iterations 1 -max-runtime 15s -iteration-timeout 10s -execute
```

```powershell
go run ./cmd/live-loop -config configs/live.local.yaml -plan-file artifacts/live-order-plan.json -readiness-file artifacts/live-readiness.json -kill-switch-file artifacts/risk-kill-switch-state.json -audit-file artifacts/live-loop-audit.json -deploy-check-file artifacts/live-deploy-check.json -ops-report-file artifacts/live-ops-report.json -subaccount-confirmed -max-initial-live-capital-usdt 100 -max-iterations 1 -max-runtime 15s -iteration-timeout 10s -execute
```

To build and verify the same first-order handoff bundle with one operator command, use `live-first-order-check`. It first snapshots persistent Kill Switch state to `risk-kill-switch-state.json`, then runs `live-order-plan`, `live-readiness`, `live-loop-audit`, `live-deploy-check`, a hard-gated `live-ops-report -fail-on-non-clear`, and final `live-handoff-verify -kill-switch-file -ops-report-file`, writes artifacts under `-artifact-dir`, then prints the exact armed `live-loop` command including `-kill-switch-file` and `-ops-report-file`. It does not run `live-loop` and cannot place an order by itself. Add `-position-drift` only when the generated ops report should also make private exchange position reads before first order handoff; add `-activate-kill-switch-on-position-drift-blocked` only when a `BLOCKED` exchange-vs-DB drift result should immediately persist an active Kill Switch event:
...
```powershell
go run ./cmd/live-first-order-check -config configs/live.local.yaml -decision-id risk_decision_live_001 -symbol BTCUSDT -artifact-dir artifacts/live-first-order -position-drift -position-drift-symbols BTCUSDT,ETHUSDT -activate-kill-switch-on-position-drift-blocked -subaccount-confirmed -execute
```
...
Use `live-ops-report` for a read-only operational snapshot before the next armed action. It checks the current Kill Switch, pending LIVE decisions, recent live-loop audit verdict, and optionally the fresh first-order review artifact. Add `-position-drift` when the report should also read current private exchange positions and compare them against the latest PostgreSQL baselines. Add `-activate-kill-switch-on-position-drift-blocked` when a `BLOCKED` drift result should immediately persist an active Kill Switch event before the command exits non-zero. The command writes `CLEAR`, `ATTENTION`, or `BLOCKED` into a durable JSON artifact; by default it exits successfully even for non-clear statuses so the report is still captured, `-fail-on-blocked` turns only `BLOCKED` into a hard gate, and `-fail-on-non-clear` fails on both `ATTENTION` and `BLOCKED`:
...
```powershell
go run ./cmd/live-ops-report -config configs/live.local.yaml -symbol BTCUSDT -position-drift -position-drift-symbols BTCUSDT,ETHUSDT -activate-kill-switch-on-position-drift-blocked -fail-on-blocked -artifact-path artifacts/live-ops-report.json
```

Pass the saved ops artifact to the next armed `live-loop` when you want this operational snapshot, including optional position drift, to be enforced as a pre-exchange gate.

Run `live-position-drift` when you need to compare current exchange positions against the latest persisted DB position baselines. It is read-only by default, uses private Bybit position reads plus PostgreSQL history, ignores volatile mark-price/PnL noise, reports stale/missing DB baselines as `ATTENTION`, and reports exposure drift or `LIQ`/`ADL` exchange status as `BLOCKED`. Add `-activate-kill-switch-on-blocked` only when a `BLOCKED` drift report should persist an active Kill Switch event:
...
make live-handoff-verify CONFIG=configs/live.local.yaml LIVE_PLAN_FILE=artifacts/live-order-plan.json LIVE_READINESS_FILE=artifacts/live-readiness.json LIVE_KILL_SWITCH_ARTIFACT=artifacts/risk-kill-switch-state.json LIVE_AUDIT_ARTIFACT=artifacts/live-loop-audit.json LIVE_DEPLOY_ARTIFACT=artifacts/live-deploy-check.json LIVE_OPS_REPORT_FILE=artifacts/live-ops-report.json LIVE_DECISION_ID=risk_decision_live_001 LIVE_SUBACCOUNT_CONFIRMED=1 LIVE_EXECUTE=1
...
make live-first-order-check CONFIG=configs/live.local.yaml LIVE_DECISION_ID=risk_decision_live_001 LIVE_PENDING_SYMBOL=BTCUSDT LIVE_OPS_POSITION_DRIFT=1 LIVE_OPS_POSITION_DRIFT_SYMBOLS=BTCUSDT,ETHUSDT LIVE_OPS_ACTIVATE_KILL_SWITCH_ON_POSITION_DRIFT_BLOCKED=1 LIVE_SUBACCOUNT_CONFIRMED=1 LIVE_EXECUTE=1
make live-first-order-review CONFIG=configs/live.local.yaml LIVE_FIRST_ORDER_ARTIFACT_DIR=artifacts/live-first-order
make live-ops-report CONFIG=configs/live.local.yaml LIVE_OPS_SYMBOL=BTCUSDT LIVE_OPS_FIRST_ORDER_REVIEW_ARTIFACT=artifacts/live-first-order/live-first-order-review.json LIVE_OPS_ARTIFACT=artifacts/live-ops-report.json LIVE_OPS_FAIL_ON_NON_CLEAR=1
...
make live-loop CONFIG=configs/live.local.yaml LIVE_PLAN_FILE=artifacts/live-order-plan.json LIVE_READINESS_FILE=artifacts/live-readiness.json LIVE_KILL_SWITCH_ARTIFACT=artifacts/risk-kill-switch-state.json LIVE_AUDIT_ARTIFACT=artifacts/live-loop-audit.json LIVE_DEPLOY_ARTIFACT=artifacts/live-deploy-check.json LIVE_OPS_REPORT_FILE=artifacts/live-ops-report.json LIVE_SUBACCOUNT_CONFIRMED=1 LIVE_EXECUTE=1
