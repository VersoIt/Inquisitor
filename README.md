...
To build and verify the same first-order handoff bundle with one operator command, use `live-first-order-check`. It first snapshots persistent Kill Switch state to `risk-kill-switch-state.json`, then runs `live-order-plan`, `live-readiness`, `live-loop-audit`, `live-deploy-check`, `live-handoff-verify -kill-switch-file`, and a hard-gated `live-ops-report -fail-on-non-clear`, writes artifacts under `-artifact-dir`, then prints the exact armed `live-loop` command including `-kill-switch-file` and `-ops-report-file`. It does not run `live-loop` and cannot place an order by itself. Add `-position-drift` only when the generated ops report should also make private exchange position reads before first order handoff; add `-activate-kill-switch-on-position-drift-blocked` only when a `BLOCKED` exchange-vs-DB drift result should immediately persist an active Kill Switch event:
