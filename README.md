...
For the safest handoff from preview to execution, pass both JSON artifacts to the armed `live-loop`: the order plan from `live-order-plan` and the PASS checklist from `live-readiness`. Use an explicit `-decision-id` artifact with explicit execution, or a FIFO `-select-pending` artifact with `live-loop -select-pending`; mixed source modes are rejected before configuration, database, or exchange setup. `live-handoff-verify` can also accept the optional `live-loop-audit` artifact and verify its freshness plus audit summary/verdict against the readiness artifact before any DB or exchange-capable command runs. `live-readiness` records the exact plan-file SHA-256 in the checklist, and `live-loop` recomputes it before execution. `live-loop` rejects stale readiness artifacts (`-max-readiness-age`, `10m` by default), stale plan artifacts (`-max-plan-age`, `10m` by default), readiness FAIL/checklist mismatches, plan metadata or SHA-256 mismatches, and any current PostgreSQL risk snapshot drift before startup preflight or exchange setup:
...
```powershell
go run ./cmd/live-loop -config configs/live.local.yaml -plan-file artifacts/live-order-plan.json -readiness-file artifacts/live-readiness.json -subaccount-confirmed -max-initial-live-capital-usdt 100 -max-iterations 1 -max-runtime 15s -iteration-timeout 10s -execute
```
...
make live-loop CONFIG=configs/live.local.yaml LIVE_PLAN_FILE=artifacts/live-order-plan.json LIVE_READINESS_FILE=artifacts/live-readiness.json LIVE_SUBACCOUNT_CONFIRMED=1 LIVE_EXECUTE=1
