...
func cleanupHypotheses(t *testing.T, ctx context.Context, db *sql.DB) {
	...
	`); err != nil {
		t.Fatalf("cleanup hypothesis paper equity events: %v", err)
	}
	if _, err := db.ExecContext(ctx, `
	...
	`); err != nil {
		t.Fatalf("cleanup hypothesis paper position closes: %v", err)
	}
	if _, err := db.ExecContext(ctx, `
	...
	`); err != nil {
		t.Fatalf("cleanup hypothesis paper open positions: %v", err)
	}
	if _, err := db.ExecContext(ctx, `
	...
	`); err != nil {
		t.Fatalf("cleanup hypothesis paper order fills: %v", err)
	}
	if _, err := db.ExecContext(ctx, `
	...
	`); err != nil {
		t.Fatalf("cleanup hypothesis paper order tickets: %v", err)
	}
	if _, err := db.ExecContext(ctx, `
	...
	`); err != nil {
		t.Fatalf("cleanup hypothesis paper validation trades: %v", err)
	}
	if _, err := db.ExecContext(ctx, `
	...
	`); err != nil {
		t.Fatalf("cleanup hypothesis paper validation records: %v", err)
	}
	if _, err := db.ExecContext(ctx, `
