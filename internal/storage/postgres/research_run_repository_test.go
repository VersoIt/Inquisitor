...
func cleanupResearchRuns(t *testing.T, ctx context.Context, db *sql.DB) {
	...
	`); err != nil {
		t.Fatalf("cleanup research paper equity events: %v", err)
	}
	if _, err := db.ExecContext(ctx, `
	...
	`); err != nil {
		t.Fatalf("cleanup research paper position closes: %v", err)
	}
	if _, err := db.ExecContext(ctx, `
	...
	`); err != nil {
		t.Fatalf("cleanup research paper open positions: %v", err)
	}
	if _, err := db.ExecContext(ctx, `
	...
	`); err != nil {
		t.Fatalf("cleanup research paper order fills: %v", err)
	}
	if _, err := db.ExecContext(ctx, `
